// macOS/Linux, Clang/GCC, C++17, FP32, one thread, s -> h -> d.
// Build without fast-math so reference/validation retain normal FP semantics.
// Usage: all_heads_shd_order_benchmark S H D trials target_ms seed [padding_elements=32]
...
size_t number(const char* arg) {
  ...
  if (end != s.size() || value > SIZE_MAX)
    throw std::runtime_error("Integer overflow");
  return static_cast<size_t>(value);
}

struct Free { void operator()(float* p) const { std::free(p); }};
...
// Output remains out[h*S+s], matching the h-s-d experiment.
// One runtime-stride kernel for all layouts. Inspect assembly for target-specific versioning.
// Input strides differ; Q, output and loop order are shared.

__attribute__((noinline))
void dot_all(const float* q, const float* k, float* out,
             size_t S, size_t H, size_t D,
             size_t sequence_stride, size_t head_stride) {
...
int main(int argc, char** argv) {
  try {
    if(argc != 7 && argc != 8)
      throw std::runtime_error("Usage: all_heads_shd_order_benchmark S H D trials target_ms seed [padding_elements=32]");
    ...
    if(!S || !H || !D || !trials || !target_ms || !padding || seed > UINT32_MAX)
      throw std::runtime_error("Shapes, trials, target_ms, padding must be positive; seed must be uint32");
    ...
    std::cerr << "K allocations: " << total_k_bytes << " bytes; alignment: "
              << alignment << " bytes; loop order: s-h-d\n";
    ...
    auto random_float=[&]() { return float(data_rng() >> 8)/8388608.0 - 1.0f; };
    for(size_t i = 0; i < HD; ++i)
      q.get()[i] = random_float();
    ...
    for(size_t trial = 0; trial < trials; ++trial) {
      ...
      for(size_t position=0;position<3;++position) {
        ...
        std::cout << names[l] << ',' << S << ',' << H << ',' << D << ',' << H << ",s-h-d,"
